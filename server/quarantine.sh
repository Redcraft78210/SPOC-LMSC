...
command -v clamdscan &>/dev/null || {
    echo "Erreur : clamdscan n'est pas installé. Veuillez l'installer."
    exit 1
}
...
docker ps -q --filter "name=quarantine_container" | grep -q . || {
    echo "Erreur : Le conteneur Docker 'quarantine_container' n'est pas en cours d'exécution."
    exit 1
}

# Vérifie que le fichier est infecté
# Run clamdscan and check return code
clamdscan --fdpass --quiet --infected "$FICHIER"
if [ $? -eq 1 ]; then
    echo "[+] Fichier infecté détecté : $FICHIER"

    # Chiffrement avec AES-256-CBC
    openssl enc -aes-256-cbc -salt -pbkdf2 -iter 10000 -in "$FICHIER" -out "$TMP_ENC" -pass file:"$CLE"
